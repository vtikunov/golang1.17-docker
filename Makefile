...
REGISTRY_PATH = vtikunov/golang
IMAGES_PREFIX := $(subst .,_,$(shell basename $(shell dirname $(realpath $(lastword $(MAKEFILE_LIST))))))
PUBLISH_TAGS = latest
PULL_TAG = latest
IMAGE = $(REGISTRY_HOST)/$(REGISTRY_PATH)
IMAGE_LOCAL_TAG = $(IMAGES_PREFIX)_go
...
go: up
	$(docker_compose_bin) exec -u $(shell id -u) $(CONTAINER_NAME) go $(ARGS)

shell: up
	$(docker_compose_bin) exec -u $(shell id -u) $(CONTAINER_NAME) sh

test: up
	$(docker_compose_bin) exec -u $(shell id -u) $(CONTAINER_NAME) /bin/sh -c "cd /usr/data/app/src && go test"
